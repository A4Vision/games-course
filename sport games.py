...
def test_get_game_info():
    with open("html_exam_text.txt") as file:
        func_info = get_game_info(file.read())
    assert func_info["home team"] == "ברצלונה"
    assert func_info["guest team"] == "באיירן מינכן"
    assert func_info["result"] == "8 - 2"
...
if __name__ == '__main__':
    ...
    with open("sport_games.csv", "w") as file:
        file.write("קבוצה אורחת, תוצאה\\שעת המשחק, קבוצה ביתית\n")
        for i in urls_of_games:
            html = requests.get(i).content
            game_info = get_game_info(html)

            file.write(f"{game_info['home team']}, {game_info['result']}, {game_info['guest team']}\n")
